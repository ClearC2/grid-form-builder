...
var Total = function (_Component) {
  ...
  function Total() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Total.__proto__ || Object.getPrototypeOf(Total)).call.apply(_ref, [this].concat(args))), _this), _this.componentDidUpdate = function (p) {
      ...
      var oldValue = _this.calculateNumericValue(p);
      var newValue = _this.calculateNumericValue(_this.props);
    ...
    }, _this.calculatePriceTimesDiscount = function (props) {
      props = props || _this.props;
      var _props = props,
          formValues = _props.formValues;

      var priceString = String(formValues.get('price') || '').replace(/,/g, '');
      var price = _this.toNumber(priceString);
      var discountString = String(formValues.get('discount') || '').replace(/%/g, '');
      var discount = _this.toNumber(discountString);
      var quantityString = String(formValues.get('quantity') || '').replace(/,/g, '');
      var quantity = _this.toNumber(quantityString);
      return quantity * price - discount;
    }, _this.calculateNumericValue = function (props) {
      props = props || _this.props;
      var _props$config = props.config,
          _props$config$fields = _props$config.fields,
          fields = _props$config$fields === undefined ? [] : _props$config$fields,
          formula = _props$config.formula;

      if (formula === '(quantity x price) - discount') {
        return _this.calculatePriceTimesDiscount(props);
      }
      var total = 0;
      fields.forEach(function (field) {
        var value = String(props.formValues.get(field) || '').replace(/,/g, '');
        total += _this.toNumber(value);
      });
      return total;
      ...
      return _this.formatNumber(formValues.get(config.name));
  ...
  _createClass(Total, [{
    ...
    value: function formatNumber(value) {
      return this.toNumber(value).toLocaleString('en', {
        style: 'currency',
        currency: 'USD'
      });
    }
...
exports.default = (0, _reactDnd.DropTarget)('FormBuilderDraggable', boxTarget, collect)(Total);