  ...
  function Typeahead() {
    ...
    return _ret2 = (_temp2 = (_this2 = _possibleConstructorReturn(this, (_ref2 = Typeahead.__proto__ || Object.getPrototypeOf(Typeahead)).call.apply(_ref2, [this].concat(args))), _this2), _this2.state = {
      ...
      var _config$name2 = config.name,
          ...
          url = _config$url === undefined ? '' : _config$url;
      var _keyword$category = keyword.category,
          category = _keyword$category === undefined ? null : _keyword$category;
      ...
      if (search.length > _this2.props.minChars) {
        if (url) {
          return GFBConfig.ajax.get(url + '/' + search).then(function (resp) {
            return { options: values.concat(resp.data.data) };
          });
        } else {
          return GFBConfig.ajax.get('/api/typeahead/name/' + category + '/search/' + search).then(function (resp) {
            return { options: values.concat(resp.data.data) };
          });
        }
      }
      ...
      var value = formValues.get(name, { value: '', label: '' });
