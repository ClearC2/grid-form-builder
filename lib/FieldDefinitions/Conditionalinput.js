...
var SINGLE_FIELD_INPUTS = Set(['multiselect', 'multicheckbox', 'listselect', 'typeahead']);
var MULTI_FIELD_INPUTS = Set(['input', 'date', 'datetime', 'phone']);
var ONLY_CATEGORICAL_INPUT = Set(['multicheckbox', 'multiselect', 'listselect']);

export var TEXT_INPUTS = ['textarea', 'checkbox', 'radio'];
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.handleOnChange = function (e) {
    ...
    if (_this3.inputType() === 'typeahead') {
      ...
      _this3.setState({ modalFormValues: _this3.state.modalFormValues.set(e.target.name, e.target.value) });
      if (e.target.value) {
        _this3.props.handleOnChange({ target: { name: _this3.parentFieldName(), value: List(e.target.value) } });
        _this3.setState({ modalFormValues: _this3.state.modalFormValues.set(e.target.name, e.target.value) });
      }
      ...
      if (MULTI_FIELD_INPUTS.has(_this3.inputType())) {
        _oldValue = _oldValue.setIn(['values', _this3.getEventFieldIndex(e)], typeof e.target.value === 'string' ? e.target.value : e.target.value.get('values'));
      } else {
        if (typeof e.target.value === 'string') {
          if (e.target.value === '' && _this3.inputType() === 'multiselect') {
            _oldValue = _oldValue.setIn(['values'], List());
          } else {
            _oldValue = _oldValue.setIn(['values'], e.target.value);
          }
        } else {
          _oldValue = _oldValue.setIn(['values'], e.target.value.get('values'));
        }
      }
