...
var Conditionalinput = function (_Component) {
  ...
  function Conditionalinput(props) {
    ...
    var conds = [];
    if (_this.props.conditions) {
      conds = _this.props.conditions;
    } else {
      conds = CONDITIONS;
    }
    if (_this.props.excludeConditions) {
      var excludes = (0, _immutable.Set)(_this.props.excludeConditions);
      var newConds = {};
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.inputTypeOptionsList = function (type) {
    var options = [];
    Object.keys(_this3.state.conditions).forEach(function (key) {
      if (!(0, _immutable.Set)(_this3.state.conditions[key].invalidInputTypes).has(type)) {
        options.push(key);
      }
    });
    return options;
