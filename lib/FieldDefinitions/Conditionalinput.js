...
var Conditionalinput = function (_Component) {
  ...
  function Conditionalinput(props) {
    ...
    _this.closeDialogOnEnterPress = function (event) {
      if (_this.state.showDialog && event.key === 'Enter') {
    ...
    _this.calculateModalHeight = function () {
      ...
      var nFields = ONLY_CATEGORICAL_INPUT.has(_this.getInputType()) ? 1 : Math.max(_this.state.values.size || 0, 1);
    ...
    _this.getMaxFields = function () {
      if (_this.state.formValues.get('condition', '') === 'is blank' || _this.state.formValues.get('condition', '') === 'is not blank') {
        return 0;
      }
    ...
    _this.squashValues = function (deletedIndex, list, fieldPrefix) {
      ...
      while (i + 1 < _this.state.values.size) {
        var copyField = fieldPrefix ? fieldPrefix + (i + 1) : i + 1; // field name of thing that will replace other thing
    ...
    _this.handleOnChange = function (e) {
      ...
      if (_this.props.handleOnChange) {
        ...
        var valEvent = {
          target: {
            value: _this.getInputType() === 'typeahead' ? newTypeaheadValues : newValues,
            name: _this.parentFieldName()
          }
        };
    ...
    _this.render = function () {
      ...
      var hideDisplay = _this.hideDisplay();
      var styles = {
        container: {
          display: 'flex',
          flex: 1,
...
Conditionalinput.defaultProps = {
  options: {
    all: ['contains', 'is equal to', 'is not equal to', 'does not contain', 'is between', 'is not between', 'is equal to', 'is not equal to', 'is between', 'is not between', 'contains', 'does not contain', 'is greater than', 'is less than', 'is one of', 'is not one of', 'is blank', 'is not blank'],
