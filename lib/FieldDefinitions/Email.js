...
var Email = function (_Component) {
  ...
  function Email() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Email.__proto__ || Object.getPrototypeOf(Email)).call.apply(_ref, [this].concat(args))), _this), _this.componentDidUpdate = function (p) {
      ...
      if (emailLocal.includes('..') || emailLocal.length < 1 || emailLocal[0] === '.' || emailLocal[emailLocal.length - 1] === '.') {
        return false;
      }

      var emailDomain = emailInput[1].split('.');
      ...
      var emailDomainBeginning = emailDomain[0];
      var emailDomainEnding = emailDomain[1];

      // validating domain part of the email
      if (emailDomainBeginning.match(/^[0-9]+$/) || emailDomainEnding.match(/^[0-9]+$/) || emailDomainBeginning.length < 1 || emailDomainEnding.length < 2 || emailDomainBeginning[0] === '-' || emailDomainEnding[emailDomainEnding.length - 1] === '-') {
        return false;
      }

      return true;
    }, _this.generateValidationError = function (value) {
      return !_this.emailValidation(value) && '* The field Email is not in a valid format. The field contents must follow email address standards';
    }, _this.handleOnChange = function () {}, _this.render = function () {
      ...
      placeholder = warn ? _this.generateValidationError(value) : placeholder;
      ...
      var styles = {
        ...
        placeholder: {
          ...
          marginLeft: 3,
          marginTop: -1,
          color: !_this.emailValidation(value) && '#ec1c24',
          marginRight: 5
        }
      };
