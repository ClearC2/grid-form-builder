...
var FormBuilder = function (_Component) {
  ...
  function FormBuilder() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = FormBuilder.__proto__ || Object.getPrototypeOf(FormBuilder)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      ...
      onClick(config, e);
    }, _this.handleDragDropOnInput = function (_ref2) {
      ...
      var _this$props$formSchem2 = _this.props.formSchema,
          formSchema = _this$props$formSchem2 === undefined ? Map() : _this$props$formSchem2;
      ...
      _this.updateStaticOffset(layout); // if we have a form on initial render, update static offset so next form will start correctly
    }, _this.componentDidUpdate = function (p) {
      var _this$props$formSchem3 = _this.props.formSchema,
          formSchema = _this$props$formSchem3 === undefined ? Map() : _this$props$formSchem3;
      var _p$formSchema = p.formSchema,
          pformSchema = _p$formSchema === undefined ? Map() : _p$formSchema;

      formSchema = typeof formSchema.toJS === 'function' ? formSchema.toJS() : formSchema;
      pformSchema = typeof pformSchema.toJS === 'function' ? pformSchema.toJS() : pformSchema;
      var _formSchema4 = formSchema,
          form = _formSchema4.form,
          jsonschema = _formSchema4.jsonschema;
      var _pformSchema = pformSchema,
          pform = _pformSchema.form,
          pjsonschema = _pformSchema.jsonschema;

      jsonschema = jsonschema || form || {};
      pjsonschema = pjsonschema || pform || {};
      var _jsonschema4 = jsonschema,
          _jsonschema4$layout = _jsonschema4.layout,
          layout = _jsonschema4$layout === undefined ? [] : _jsonschema4$layout;
      var _pjsonschema = pjsonschema,
          _pjsonschema$layout = _pjsonschema.layout,
          playout = _pjsonschema$layout === undefined ? [] : _pjsonschema$layout;

      if (layout.length !== playout.length) {
        _this.updateMyOffset(layout); // if form was hot swapped or async loaded, just flat out treat it like a brand new form with a whole new offset
      }
    }, _this.updateMyOffset = function (layout) {
      _this.setState({ myOffset: FormBuilder.tabOffset + layout.length }, _this.updateStaticOffset(layout));
    }, _this.updateStaticOffset = function (layout) {
      ...
      var _this$state = _this.state,
          ...
          myOffset = _this$state.myOffset;

      formValues = typeof formValues.isMap === 'function' ? formValues : Map(formValues);
      formSchema = typeof formSchema.toJS === 'function' ? formSchema.toJS() : formSchema;
      var dateFields = [];
      var normalFields = [];
      var _formSchema5 = formSchema,
          form = _formSchema5.form,
          jsonschema = _formSchema5.jsonschema;

      jsonschema = jsonschema || form || {};
      var _jsonschema5 = jsonschema,
          _jsonschema5$layout = _jsonschema5.layout,
          layout = _jsonschema5$layout === undefined ? [] : _jsonschema5$layout;
