...
const getFieldSchema = (key, formSchema) => {
  ...
  if (formSchema && formSchema.jsonschema && formSchema.jsonschema.layout) {
    ...
    return _findInstanceProperty(_context = List(formSchema.jsonschema.layout)).call(_context, row => row.config.name === key);
  } else {
    return undefined;
  }
};
const getBetweenDatesValues = query => {
  ...
  return _filterInstanceProperty(_context2 = _mapInstanceProperty(_context3 = _filterInstanceProperty(query).call(query, q => {
    var _context4;
  ...
  if (schema) {
    if (schema.config && schema.config.type && Set(TEXT_INPUTS).has(schema.config.type.toLowerCase()) && c.get('comparator') !== 'is blank' && c.get('comparator') !== 'is not blank') {
      ...
      if (c.get('rawValues') !== undefined && !mergeDate) {
        ...
        var _context5;
        const values = _mapInstanceProperty(_context5 = _filterInstanceProperty(inBetweenDateValues).call(inBetweenDateValues, v => v.field === c.get('name'))).call(_context5, v => v.value);
        newFormValue = Map({
...
export const convertQueryToFormValues = function (query) {
  let clearExistingValues = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  let fValues = arguments.length > 2 ? arguments[2] : undefined;
  let formSchema = arguments.length > 3 ? arguments[3] : undefined;
  ...
  if (query) {
    ...
    if (clearExistingValues) {
      _forEachInstanceProperty(formValues).call(formValues, (v, k) => {
        if (v instanceof Map) {
    ...
    if (query.conditions) {
      ...
      _forEachInstanceProperty(_context6 = fromJS(query.conditions)).call(_context6, c => {
        if (c.get('conditions')) {
          var _context7;
          let conditions = List();
          _forEachInstanceProperty(_context7 = c.get('conditions')).call(_context7, pred => {
...
class _ConditionalTableContainer extends Component {
  constructor(props) {
    ...
    super(props);
    _this = this;
    _defineProperty(this, "convertQueryToFormValues", function (query) {
      let clearExistingValues = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      let {
        formValues
      } = _this.props;
    ...
    _defineProperty(this, "getDefaultCondition", inputType => {
      let i = 0;
      const max = CONDITIONS.length;
      while (i < max) {
        if (!Set(CONDITIONS[i].invalidInputTypes).has(inputType)) {
    ...
    _defineProperty(this, "getFieldSchema", key => {
      let {
        formSchema
      } = this.state;
      if (typeof formSchema.toJS === 'function') formSchema = formSchema.toJS();
      if (formSchema && formSchema.jsonschema && formSchema.jsonschema.layout) {
        var _context8;
        return _findInstanceProperty(_context8 = List(formSchema.jsonschema.layout)).call(_context8, row => row.config.name === key);
      } else {
        return undefined;
      }
    });
