...
var ConditionalInput = function ConditionalInput(props) {
  ...
  var hasValue = vals.size > 0 || _includesInstanceProperty(cond).call(cond, 'blank') || cond === 'today' || cond === 'this month' || cond === 'year to date' || cond === 'this quarter' || cond === 'quarter to date' || cond === 'this week' || values.getIn([name, 'dynamicValues']) && values.getIn([name, 'dynamicValues']).size || values.getIn([name, 'conditions'], List()).size > 0;
