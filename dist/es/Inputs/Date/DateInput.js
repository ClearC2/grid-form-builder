...
const DateInput = props => {
  ...
  const handleOnBlur = useCallback(e => {
    if ((type === 'month' || type === 'monthday') && manualBlurCheck) {
      const formatted = inputValue ? moment(inputValue, type === 'month' ? 'MM/YYYY' : 'MM/DD').format(inputFormat) : '';
      ...
      const validate = onChangeValidator({
        raw: inputValue,
        formatted
      });
  ...
  const handleOnCalendarChange = useCallback(e => {
    if (allowCalendarChangeEvent.current) {
      ...
      const validate = typeof onChangeValidator === 'function' ? onChangeValidator({
        raw: inputValue,
