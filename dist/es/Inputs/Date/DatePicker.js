    ...
    return () => {
      ...
      const calculateDate = dateVal => {
        ...
        if (typeof dateVal === 'string' && _startsWithInstanceProperty(dateVal).call(dateVal, 'today +')) {
          var _context;
          const daysToAdd = _parseInt(_trimInstanceProperty(_context = dateVal.split('+')[1]).call(_context), 10);
      ...
      $input.on('show.daterangepicker', () => {
        ...
        if ($portal) {
          ...
          if (!showCalendar) {
            const $calendar = _findInstanceProperty($portal).call($portal, '.calendar-table');
            if ($calendar) {
              $calendar.hide();
      ...
      $input.on('hide.daterangepicker', (e, calendar) => {
