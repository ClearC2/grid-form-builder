...
export const emailValidator = email => {
  ...
  email = _trimInstanceProperty(_context2 = String(email)).call(_context2);

  // https://www.mailboxvalidator.com/resources/articles/acceptable-email-address-syntax-rfc/
  const emailInput = email.split('@');
  const emailLocal = _trimInstanceProperty(_context3 = emailInput[0]).call(_context3);
  ...
  const domainPart = _trimInstanceProperty(_context4 = emailInput[1]).call(_context4);
...
export const uppercaseFirstLetter = string => string.charAt(0).toUpperCase() + _sliceInstanceProperty(string).call(string, 1).toLowerCase();
...
export const convertDelimitedValueIntoLabelValueArray = _ref => {
  let {
    delimit,
    delimiter,
    value,
  ...
  let values = [];
  let tempValueObject = {};
  _forEachInstanceProperty(formattedValue).call(formattedValue, (value, i) => {
  ...
  values = _mapInstanceProperty(values).call(values, value => {
    const matchingOption = _findInstanceProperty(formattedOptions).call(formattedOptions, option => {
  ...
  if (formattedOptions.length && showValidOptions) {
    ...
    _forEachInstanceProperty(values).call(values, value => {
      ...
      const option = _findInstanceProperty(formattedOptions).call(formattedOptions, option => {
        if (typeof value === 'string' || typeof value === 'number') {
          return _findInstanceProperty(delimit).call(delimit, field => {
...
export const convertLabelValueArrayIntoDelimitedValue = _ref2 => {
  let {
    delimit,
    delimiter,
    stringify,
  ...
  if (stringify) {
    ...
    if (delimiter) {
      if (delimit && _Array$isArray(delimit)) {
        ...
        _forEachInstanceProperty(value).call(value, option => {
          _forEachInstanceProperty(delimit).call(delimit, field => {
